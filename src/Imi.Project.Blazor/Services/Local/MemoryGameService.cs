...
namespace Imi.Project.Blazor.Services
{
    public class MemoryGameService : IMemoryGameService
    {
        ...
        public async Task Flip(MemoryGameCard card)
        {
            ...
            if (FlippedCards.Count() == MemoryGameSettings.NumberOfCards)
            {
                FlippedCards.Clear();
                await Task.Delay(600);
                Wins++;
        ...
        public Color[] GetRandomColorList()
        {
        ...
        private List<MemoryGameCard> GetCardPairs()
        {
            ...
            for (int i = 1; i < (MemoryGameSettings.NumberOfPairs + 1); i++)
            {
        ...
        private List<List<MemoryGameCard>> GetCardsList(List<MemoryGameCard> cardPairs)
        {
            ...
            for (int i = 0; i < MemoryGameSettings.NumberOfCards; i++)
            {
                if (i % MemoryGameSettings.NumberOfRows == 0)
                {
                    cards.Add(new List<MemoryGameCard>());
                }
                cards[i / MemoryGameSettings.NumberOfRows].Add(cardPairs[i]);
            }
            return cards;
        }
